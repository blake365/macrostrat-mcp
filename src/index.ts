...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
	...
	const { lat, lng, adjacents = true, age } = request.params.arguments as any;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
	...
	if (request.params.name === "columns") {
		const { lat, lng, adjacents } = request.params.arguments as any;
		const response = await fetch(
			`${getApiEndpoint("columns")}?${new URLSearchParams({
				lat: lat.toString(),
				lng: lng.toString(),
				adjacents: adjacents,
			})}`,
		...
		const { lat, lng } = request.params.arguments as any;
		const response = await fetch(
			`${getApiEndpoint("mapUnits")}?${new URLSearchParams({
				lat: lat.toString(),
				lng: lng.toString(),
			})}`,
		);
...
async function getUnits(lat: number, lng: number, age?: number) {
	...
	if (age) {
		...
		const data = (await resp.json()) as MacrostratUnit[];
		return data;
	}
	...
	let sendData = data.success.data as MacrostratUnit[];
...
async function getColumns(lat: number, lng: number, adjacents = false) {
	const params = {
		lat: lat.toString(),
		lng: lng.toString(),
