...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
	...
	const { lat, lng, radius = 100000 } = request.params.arguments as any;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
	...
	if (request.params.name === "columns") {
		const response = await fetch(
			`${API_BASE}/columns?${new URLSearchParams(params)}`,
		);
		data = await response.json();
	} else if (request.params.name === "units") {
		const response = await fetch(
			`${API_BASE}/units?${new URLSearchParams(params)}`,
		);
		data = await response.json();
...
async function getUnits(lat: number, lng: number) {
...
async function getColumns(lat: number, lng: number, radius = 100000) {
	const params = {
		lat: lat.toString(),
		lng: lng.toString(),
...
function summarizeUnits(units: any[]) {
	console.log(units);
